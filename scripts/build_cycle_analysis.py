...
def leader_strength(emotion: dict) -> dict:
    ...
    if open_count is not None:
        stability_value, stability_score, stability_basis, stability_proxy = open_count, falling(open_count, 0, 5), "开板次数", False
    elif retention is not None:
        stability_value, stability_score, stability_basis, stability_proxy = retention, rising(retention, 20, 100), "当前封单额/峰值封单额", False
        ...
        stability_score = 60 + rising(seal_ratio, 1, 50) * .2
        stability_basis = "收盘仍封住+封单额/成交额代理"
        stability_proxy = True
    else:
        stability_value, stability_score, stability_basis, stability_proxy = None, None, "数据缺失", False
    result = weighted([
        ...
        {"metric": "封板稳定性", "value": stability_value, "weight": 25,
         ...
         "basis": stability_basis, "proxy": stability_proxy},
